...
if __name__ == "__main__":
    ...
    parser.add_argument('--lmbda_gp', default=10., type=float)
    args = parser.parse_args()

    if not os.path.exists("./job_scripts_w1"):
        os.makedirs("./job_scripts_w1")
    if not os.path.exists(f"./python_outputs_w1/Exp{args.ExpID}"):
        os.makedirs(f"./python_outputs_w1/Exp{args.ExpID}")
    ...
    for idx, (env_name, fixed_rollout_len, noise_dim) in enumerate(envs_list):
        ...
        if idx % len(gpu_devive) == 0:
            if len(gpu_devive) == 1:
                job_file_name = f"./job_scripts_w1/run_Exp{args.ExpID}_{env_name}.sh"
            else:
                job_file_name = f"./job_scripts_w1/run_Exp{args.ExpID}_{idx // len(gpu_devive) + 1}.sh"
        ...
        run_cmd += f"  OMP_NUM_THREADS={args.num_thread} MKL_NUM_THREADS={args.num_thread} python w1_main.py \\\n"
        run_cmd += f"  --device 'cuda:{gpu_devive[idx % len(gpu_devive)]}' \\\n"
        run_cmd += f'  --log_dir "./results_w1/" \\\n'
        ...
        run_cmd += f"  --lmbda_gp {args.lmbda_gp} \\\n"
        run_cmd += f"  --ExpID {args.ExpID}"
        if len(gpu_devive) == 1:
            run_cmd += f' > "./python_outputs_w1/Exp{args.ExpID}/{env_name}_$i.txt" & \n'
        else:
            run_cmd += f' > "./python_outputs_w1/Exp{args.ExpID}/task{idx // len(gpu_devive) + 1}{idx % len(gpu_devive) + 1}_$i.txt" & \n'
